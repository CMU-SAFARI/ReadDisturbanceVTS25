...
if __name__ == "__main__":
  ...
  # i.set_module_temperature(module, 80)
  p = DDR4(0, "XDMA")
  p.resetFPGA()
  p.setAREF(False)
      ...
      for dp_id in range(2):
        ...
        for hc in [7500]:
          progs = [
            ...
            singleside_hammer(bank_id, upper_aggr_row.logical_id, hc, 260, 0),
        ...
        for hc in [7500]:
          progs = [
            ...
            singleside_hammer(bank_id, lower_aggr_row.logical_id, hc, 260, 0),
            read_rows(bank_id, [vic_row.logical_id]),
          ]
          p.execute(progs)
          bitflips = get_bitflips(p, [vic_row.wr_pattern])
  ...
  results_df.to_csv(f"{data_prefix}/{module}_rd_rp.csv", index=False)
  with gzip.open(f"{data_prefix}/{module}_rd_rp_bitflips.gz", 'wb') as f:
