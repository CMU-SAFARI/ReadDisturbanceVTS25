...
DRAMBender::Program tWR_test(int bank, int row_id, int col_id, const WriteWord_t& wr_word, int tWR) {
  ...
  // Write to row with data pattern
  for(uint i = 0 ; i < 16 ; i++) {
    p.add_inst(SMC_LI(wr_word, ReservedReg::PATTERN_REG));
  ...
  // Write to col_id with opposite data pattern
  p.add_inst(SMC_LI(col_id*8, ReservedReg::CAR));
  ...
  p.add_mininst(SMC_WRITE(ReservedReg::BAR, 0, ReservedReg::CAR, 0, 0, 0), tWR);
  p.add_mininst(SMC_PRE(ReservedReg::BAR, 0, 0), 10);
  p.pack_minprogram();
